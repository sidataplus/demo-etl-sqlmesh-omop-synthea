...
LEFT JOIN omop.CONCEPT p ON c.CONDITION_CONCEPT_ID = p.CONCEPT_ID
...
JOIN omop.concept c ON t.CONDITION_CONCEPT_ID = c.concept_id
...
-- Description: Check that concepts in 'CONDITION_OCCURRENCE.CONDITION_CONCEPT_ID' are standard and valid.
        ...
        SELECT t.*
FROM omop.CONDITION_OCCURRENCE t
LEFT JOIN omop.concept c ON t.CONDITION_CONCEPT_ID = c.concept_id
...
-- Description: Check for orphaned foreign keys in 'CONDITION_OCCURRENCE.CONDITION_SOURCE_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONDITION_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.CONDITION_SOURCE_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check for orphaned foreign keys in 'CONDITION_OCCURRENCE.CONDITION_STATUS_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONDITION_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.CONDITION_STATUS_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check that concepts in 'CONDITION_OCCURRENCE.CONDITION_STATUS_CONCEPT_ID' belong to the 'Condition Status' domain.
        ...
        SELECT t.*
FROM omop.CONDITION_OCCURRENCE t
JOIN omop.concept c ON t.CONDITION_STATUS_CONCEPT_ID = c.concept_id
...
-- Description: Check that concepts in 'CONDITION_OCCURRENCE.CONDITION_STATUS_CONCEPT_ID' are standard and valid.
        ...
        SELECT t.*
FROM omop.CONDITION_OCCURRENCE t
LEFT JOIN omop.concept c ON t.CONDITION_STATUS_CONCEPT_ID = c.concept_id
...
-- Description: Check for orphaned foreign keys in 'CONDITION_OCCURRENCE.CONDITION_TYPE_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONDITION_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.CONDITION_TYPE_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check that concepts in 'CONDITION_OCCURRENCE.CONDITION_TYPE_CONCEPT_ID' belong to the 'Type Concept' domain.
        ...
        SELECT t.*
FROM omop.CONDITION_OCCURRENCE t
JOIN omop.concept c ON t.CONDITION_TYPE_CONCEPT_ID = c.concept_id
...
-- Description: Check that concepts in 'CONDITION_OCCURRENCE.CONDITION_TYPE_CONCEPT_ID' are standard and valid.
        ...
        SELECT t.*
FROM omop.CONDITION_OCCURRENCE t
LEFT JOIN omop.concept c ON t.CONDITION_TYPE_CONCEPT_ID = c.concept_id
...
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4090861
)
  AND p.gender_concept_id <> 8507;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4025213
)
  AND p.gender_concept_id <> 8507;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4095793
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 443343
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4024004
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4172857
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 444094
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 197810
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.CONDITION_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4158481
)
  AND p.gender_concept_id <> 8532;;
