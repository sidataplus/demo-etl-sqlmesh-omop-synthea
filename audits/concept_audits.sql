...
SELECT * FROM omop.CONCEPT WHERE CONCEPT_CLASS_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT.CONCEPT_CLASS_ID' pointing to 'CONCEPT_CLASS.CONCEPT_CLASS_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT c
LEFT JOIN omop.CONCEPT_CLASS p ON c.CONCEPT_CLASS_ID = p.CONCEPT_CLASS_ID
...
SELECT * FROM omop.CONCEPT WHERE CONCEPT_CODE IS NULL;
...
SELECT * FROM omop.CONCEPT WHERE CONCEPT_ID IS NULL;
...
SELECT * FROM omop.CONCEPT WHERE CONCEPT_NAME IS NULL;
...
SELECT * FROM omop.CONCEPT WHERE DOMAIN_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT.DOMAIN_ID' pointing to 'DOMAIN.DOMAIN_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT c
LEFT JOIN omop.DOMAIN p ON c.DOMAIN_ID = p.DOMAIN_ID
...
SELECT * FROM omop.CONCEPT WHERE VALID_END_DATE IS NULL;
...
SELECT * FROM omop.CONCEPT WHERE VALID_START_DATE IS NULL;
...
SELECT * FROM omop.CONCEPT WHERE VALID_START_DATE > VALID_END_DATE;
...
SELECT * FROM omop.CONCEPT WHERE VOCABULARY_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT.VOCABULARY_ID' pointing to 'VOCABULARY.VOCABULARY_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT c
LEFT JOIN omop.VOCABULARY p ON c.VOCABULARY_ID = p.VOCABULARY_ID
