...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE CONCEPT_ID_1 IS NULL;
...
LEFT JOIN omop.CONCEPT p ON c.CONCEPT_ID_1 = p.CONCEPT_ID
...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE CONCEPT_ID_2 IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT_RELATIONSHIP.CONCEPT_ID_2' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT_RELATIONSHIP c
LEFT JOIN omop.CONCEPT p ON c.CONCEPT_ID_2 = p.CONCEPT_ID
...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE RELATIONSHIP_ID IS NULL;
...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE VALID_END_DATE IS NULL;
...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE VALID_START_DATE IS NULL;
...
SELECT * FROM omop.CONCEPT_RELATIONSHIP WHERE VALID_START_DATE > VALID_END_DATE;
