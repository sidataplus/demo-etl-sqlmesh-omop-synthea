...
SELECT * FROM omop.CONCEPT_SYNONYM WHERE CONCEPT_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT_SYNONYM.CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT_SYNONYM c
LEFT JOIN omop.CONCEPT p ON c.CONCEPT_ID = p.CONCEPT_ID
...
SELECT * FROM omop.CONCEPT_SYNONYM WHERE CONCEPT_SYNONYM_NAME IS NULL;
...
SELECT * FROM omop.CONCEPT_SYNONYM WHERE LANGUAGE_CONCEPT_ID IS NULL;
