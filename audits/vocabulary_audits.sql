...
SELECT * FROM omop.VOCABULARY WHERE VOCABULARY_CONCEPT_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'VOCABULARY.VOCABULARY_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.VOCABULARY c
LEFT JOIN omop.CONCEPT p ON c.VOCABULARY_CONCEPT_ID = p.CONCEPT_ID
...
SELECT * FROM omop.VOCABULARY WHERE VOCABULARY_ID IS NULL;
...
SELECT * FROM omop.VOCABULARY WHERE VOCABULARY_NAME IS NULL;
