...
LEFT JOIN omop.CONCEPT p ON c.MODIFIER_CONCEPT_ID = p.CONCEPT_ID
...
LEFT JOIN omop.concept c ON t.MODIFIER_CONCEPT_ID = c.concept_id
...
-- Description: Check for orphaned foreign keys in 'PROCEDURE_OCCURRENCE.PROCEDURE_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.PROCEDURE_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.PROCEDURE_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check that concepts in 'PROCEDURE_OCCURRENCE.PROCEDURE_CONCEPT_ID' belong to the 'Procedure' domain.
        ...
        SELECT t.*
FROM omop.PROCEDURE_OCCURRENCE t
JOIN omop.concept c ON t.PROCEDURE_CONCEPT_ID = c.concept_id
...
-- Description: Check that concepts in 'PROCEDURE_OCCURRENCE.PROCEDURE_CONCEPT_ID' are standard and valid.
        ...
        SELECT t.*
FROM omop.PROCEDURE_OCCURRENCE t
LEFT JOIN omop.concept c ON t.PROCEDURE_CONCEPT_ID = c.concept_id
...
-- Description: Check for orphaned foreign keys in 'PROCEDURE_OCCURRENCE.PROCEDURE_SOURCE_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.PROCEDURE_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.PROCEDURE_SOURCE_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check for orphaned foreign keys in 'PROCEDURE_OCCURRENCE.PROCEDURE_TYPE_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.PROCEDURE_OCCURRENCE c
LEFT JOIN omop.CONCEPT p ON c.PROCEDURE_TYPE_CONCEPT_ID = p.CONCEPT_ID
...
-- Description: Check that concepts in 'PROCEDURE_OCCURRENCE.PROCEDURE_TYPE_CONCEPT_ID' belong to the 'Type Concept' domain.
        ...
        SELECT t.*
FROM omop.PROCEDURE_OCCURRENCE t
JOIN omop.concept c ON t.PROCEDURE_TYPE_CONCEPT_ID = c.concept_id
...
-- Description: Check that concepts in 'PROCEDURE_OCCURRENCE.PROCEDURE_TYPE_CONCEPT_ID' are standard and valid.
        ...
        SELECT t.*
FROM omop.PROCEDURE_OCCURRENCE t
LEFT JOIN omop.concept c ON t.PROCEDURE_TYPE_CONCEPT_ID = c.concept_id
...
WHERE t.PROCEDURE_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4041261
)
  AND p.gender_concept_id <> 8532;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.PROCEDURE_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4250917
)
  AND p.gender_concept_id <> 8507;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.PROCEDURE_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4077750
)
  AND p.gender_concept_id <> 8507;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.PROCEDURE_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4043199
)
  AND p.gender_concept_id <> 8507;;
...
JOIN omop.person p ON t.person_id = p.person_id
WHERE t.PROCEDURE_CONCEPT_ID IN (
    SELECT descendant_concept_id FROM omop.concept_ancestor WHERE ancestor_concept_id = 4040577
)
  AND p.gender_concept_id <> 8507;;
