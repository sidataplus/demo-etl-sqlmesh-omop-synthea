...
SELECT * FROM omop.CONCEPT_ANCESTOR WHERE ANCESTOR_CONCEPT_ID IS NULL;
...
LEFT JOIN omop.CONCEPT p ON c.ANCESTOR_CONCEPT_ID = p.CONCEPT_ID
...
SELECT * FROM omop.CONCEPT_ANCESTOR WHERE DESCENDANT_CONCEPT_ID IS NULL;
...
-- Description: Check for orphaned foreign keys in 'CONCEPT_ANCESTOR.DESCENDANT_CONCEPT_ID' pointing to 'CONCEPT.CONCEPT_ID'.
        ...
        SELECT c.*
FROM omop.CONCEPT_ANCESTOR c
LEFT JOIN omop.CONCEPT p ON c.DESCENDANT_CONCEPT_ID = p.CONCEPT_ID
...
SELECT * FROM omop.CONCEPT_ANCESTOR WHERE MAX_LEVELS_OF_SEPARATION IS NULL;
...
SELECT * FROM omop.CONCEPT_ANCESTOR WHERE MIN_LEVELS_OF_SEPARATION IS NULL;
