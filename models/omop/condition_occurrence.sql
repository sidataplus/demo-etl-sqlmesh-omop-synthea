...
SELECT
    ...
    CAST(NULL AS TIMESTAMP) AS condition_start_datetime, -- DBT model casts NULL
    c.condition_stop_date::DATE AS condition_end_date,
    CAST(NULL AS TIMESTAMP) AS condition_end_datetime, -- DBT model casts NULL
    32827::INT AS condition_type_concept_id, -- Hardcoded in DBT model
    0::INT AS condition_status_concept_id, -- Hardcoded in DBT model
    CAST(NULL AS VARCHAR(20)) AS stop_reason, -- DBT model casts NULL to varchar
    ...
    CAST(NULL AS VARCHAR(50)) AS condition_status_source_value -- DBT model casts NULL to varchar
