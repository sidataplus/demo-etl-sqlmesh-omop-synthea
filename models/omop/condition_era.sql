MODEL (
  ...
  column_descriptions (
    condition_era_id = 'A unique identifier for each Condition Era.',
    ...
    condition_era_start_date = 'The start date for the Condition Era.',
    condition_era_end_date = 'The end date for the Condition Era.',
    condition_occurrence_count = 'The number of Condition Occurrences used to construct the Condition Era.'
  )
);

-- Logic adapted from https://github.com/OHDSI/ETL-CMS/blob/master/SQL/create_CDMv5_condition_era.sql
-- and dbt-synthea/models/omop/condition_era.sql
...
SELECT
    ...
    , CAST(era_end_date AS DATE) AS condition_era_end_date -- Replaced dbt.cast
