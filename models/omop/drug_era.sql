MODEL (
  ...
  column_descriptions (drug_era_id = 'A unique identifier for each Drug Era.', person_id = 'A foreign key identifier to the Person.', drug_concept_id = 'A foreign key identifier to the Drug Concept representing the ingredient.', drug_era_start_date = 'The start date of the Drug Era.', drug_era_end_date = 'The end date of the Drug Era.', drug_exposure_count = 'The number of drug exposure events contributing to the era.', gap_days = 'The number of days allowed between exposures to merge them into a single era.')
);
...
WITH ctePreDrugTarget AS (
  ...
  SELECT
    ...
    COALESCE(
      ...
      NULLIF(
        d.drug_exposure_start_date + /* dbt.dateadd equivalent */ INTERVAL (d.days_supply) DAY,
        d.drug_exposure_start_date
      ),
...
SELECT
  ...
  DATE_DIFF(
    'DAY',
    min_drug_sub_exposure_start_date::DATE /* dbt.cast equivalent */,
    drug_era_end_date::DATE /* dbt.cast equivalent */
